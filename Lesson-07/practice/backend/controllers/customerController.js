...
// Generate a new apiKey for a customer and persist it
export const generateApiKey = async (customerInfo) => {
    const randomString = crypto.randomBytes(4).toString('hex');
    const apiKey = `web-$${customerInfo._id}$-$${customerInfo.email}$-$${randomString}$`;

    customerInfo.apiKey = apiKey;
    await customerInfo.save();

    return apiKey;
};

// Get an apiKey for a customer
export const getApiKey = async (req, res) => {
    try {
        ...
        const apiKey = await generateApiKey(customerInfo);

        res.status(200).json({
            message: "Get apiKey successfull",
            data: { apiKey },
            success: true
        });
